<template>
  <div class="login bgfff ls1">
    ...
    <div class="pl40 pr40">
      ...
      <div class="item bor_b">
        <input :type="passwordType" v-model="password" @focus="close2=true" @blur="close2=false" placeholder="请输入新密码（最少6位）">
        <p>
      ...
      <p class="login_btn" :class="{on: phone&&verificationType&&password}" @click="login">确定</p>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    let ph = this.phone
      ...
      if (phoneNumber(ph)) {
        const params = {
          ...
          verify_type: 'retrieve' // register=>注册 , verify=>快捷登录 , retrieve=>找回密码
        }
        if (this.isGetCode) {
    ...
    let ph = this.phone
      ...
      if (this.code.length === 6) {
        const params = {
          ...
          verify_type: 'retrieve' // 验证码类型 : register=>注册 , verify=>快捷登录 , retrieve=>找回密码
        }
    ...
    let ph = this.phone
      ph = ph.replace(/\s*/g, "")
      if (ph && this.verificationType && this.password) {
        if (this.password.length > 5) {
          ...
          this.fetch.retrievePwd(params).then((res) => {
            if (res.code === 200) {
              let time = 1e3
              this.throttle(() => { // 信息提示
